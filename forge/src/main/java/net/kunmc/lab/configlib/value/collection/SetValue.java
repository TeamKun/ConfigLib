...
public abstract class SetValue<E, U extends SetValue<E, U>> extends CollectionValue<Set<E>, E, U> implements Iterable<E> {
    ...
    @Override
    protected boolean validateForAdd(String entryName, Set<E> element, CommandSource sender) {
        return !value.containsAll(element);
    }

    @Override
    protected String invalidValueMessageForAdd(String entryName, Set<E> element, CommandSource sender) {
        return String.format("%sはすでに%sに追加されています.", elementToString(((E[]) element.toArray())[0]), entryName);
    }

    @Override
    protected boolean validateForRemove(String entryName, Set<E> element, CommandSource sender) {
        return value.containsAll(element);
    }

    @Override
    protected String invalidValueMessageForRemove(String entryName, Set<E> element, CommandSource sender) {
